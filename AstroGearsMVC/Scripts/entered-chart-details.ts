...
module EnteredChartDetails {
    var listing: boolean = false;
    var aspects: boolean = false;
...
$('#saveNewAsteroids').click(function () {
    ...
    export function ProcessNewAsteroidOrFixedStar(celestialObjectName, i) {
        ...
        var jqxhr = $.getJSON("/CelestialObjects/CelestialObjectExists", { celestialObjectName: celestialObjectName }).done(function (data) {
            ...
            if (IsInRange($('#newAsteroid' + i + 'CoordinatesDegrees').val(), 0, 29) && $('#newAsteroid' + i + 'CoordinatesSigns').val() !== '---'
                ...
                SaveNewAsteroidOrFixedStar(
                    ...
                    $('#newAsteroid' + i + 'CoordinatesSeconds').val(),
                    $('#newAsteroid' + i + 'CoordinatesSigns').val(),
                    $('#newAsteroid' + i + 'CoordinatesOrientations').val());
            }
        }).fail(function (jqxhr, textStatus, error) {
                var err = textStatus + ", " + error;
    ...
    export function GetCelestialObjectId(celestialObjectName) {
    ...
    export function SaveNewAsteroidOrFixedStar(chartId, celestialObjectName, celestialObjectId, degrees, minutes, seconds, sign, orientation) {
        var jqxhr = $.getJSON("/EnteredCharts/CreateChartObjectForEnteredChart", {
    ...
    $('#editHouses').click(function () {
        $.ajaxSetup({ cache: false });
        var chartId = $('#enteredchartid').val(),
    ...
    export function SaveHouseSystemChanges() {
        $.ajaxSetup({ cache: false });
        ...
        var postEnds: JQueryPromise<any>[] = [];
    ...
    export function GetHouseListing(chartId) {
        $.ajaxSetup({ cache: false });
    ...
    export function HideAddButtons(chartId) {
    ...
    export function GetAspectsForArabicChart(arabicPartName, arabicPartCoordinates) {
        listing = false;
        aspects = true;
        $.ajaxSetup({ cache: false });
    ...
    export function GetAspectsForAngleChart(angleId, angleName, angleCoordinates) {
        listing = false;
        aspects = true;
        $.ajaxSetup({ cache: false });
    ...
    export function GetAspectsForDraconicChart(draconicName, draconicCoordinates) {
        listing = false;
        aspects = true;
        $.ajaxSetup({ cache: false });
    ...
    export function GetAsteroidAndFixedStartAutocomplete() {
